...
void draw_gui()
{
   ...
   if (ImGui::SliderFloat("Speed", &speed, -0.005f, 0.005f)){

       glUniform1f(3, speed);
   }


   static float speedSlider = 0.5f;
   if(ImGui::SliderFloat("Slider", &speedSlider, 0.0f, 1.0f))
   {
      glUniform1f(2, speedSlider);
   }
...
void reload_shader()
{
   ...
   {
      glClearColor(0.1f, 0.35f, 0.35f, 0.0f);
...
void initOpenGl()
{
   ...
   glEnable(GL_BLEND);
   glBlendFunc(GL_SRC_ALPHA, GL_ZERO); //additive alpha blending
